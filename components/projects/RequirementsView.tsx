...
import { projectsApi, requirementsRunsApi } from '@/lib/api'
import { DriveEmbed } from '@/components/projects/DriveEmbed'
import { RequirementsRunsList } from '@/components/projects/RequirementsRunsList'
...
import { cn, isTerminalStatus } from '@/lib/utils'
...
export function RequirementsView({ projectId, project }: RequirementsViewProps) {
  ...
  // ── Runs query (for run button label) ─────────────────────────────────────
  ...
  const updateUrlMutation = useMutation({
    mutationFn: async () => {
      const trimmedUrl = urlInput.trim() || null
      let name: string | null = null
      if (trimmedUrl) {
        try {
          const res = await fetch(`/api/drive-filename?url=${encodeURIComponent(trimmedUrl)}`)
          const data = await res.json()
          name = data.name ?? null
        } catch {
          // non-fatal — save without name
        }
      }
      return projectsApi.update(projectId, {
        requirements_input_drive_url: trimmedUrl,
        requirements_input_drive_name: name,
      })
    },
    onSuccess: () => {
  ...
  const handleSaveUrl = () => updateUrlMutation.mutate()
  ...
  const handleSelectRun = (run: RequirementsRun, mode: 'input' | 'output') => {
    setSelectedRun(run)
    setViewMode(mode)
  }
  ...
  return (
    <Group orientation="horizontal" className="h-full">
      {/* ── Left panel ── */}
      <Panel defaultSize={350} minSize={150} className="flex flex-col overflow-hidden border-r border-border">
                ...
                <span className="text-xs text-muted-foreground truncate flex-1 min-w-0">
                  {currentDriveUrl ? (
                    <span className="text-foreground">
                      {project.requirements_input_drive_name ?? currentDriveUrl.split('/d/')[1]?.slice(0, 20) + '…'}
                    </span>
                  ) : (
